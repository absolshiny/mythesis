...
void odometria(int v_mot1, int vmot_2, float xold, float yold, float tethaold)
{
  ...
  float Ss;
  float Ste;
  //float rpsl=  ((v_mot1*0.0547889743333222111)/60.0);
  //float rpsr = ((vmot_2*0.0547889743333222111)/60.0);
  float rpsl=  ((v_mot1*0.0370396955818)/60.0);
  float rpsr = ((vmot_2*0.0370396955818)/60.0);
  float el = (rpsl*2.0*PI_2)*espera1/1000;
  float er = (rpsr*2.0*PI_2)*espera1/1000;
  Ss=(r/2)*(er+el);
  Ste=(r/d)*(er-el);
